...
class WC_Subscription_Validation_Message extends WC_Validation_Message {
	...
	protected function init_error_messages() {
		...
		$this->error_messages[ IntroductoryRateBillingPeriods::ID ] = __(
			'The number of billing periods after a subscription ends must exceed the number of introductory rate billing periods.',
	...
	protected function setFieldLabels() {
		$this->field_labels = array(
			...
			'wgc_plan_introductory_rate_amount'          => __(
				'Introductory Rate Bill Amount',
				'elavon-converge-gateway'
			),
			'wgc_plan_introductory_rate_billing_periods' => __(
				'Introductory Rate Billing Periods',
				'elavon-converge-gateway'
			),
